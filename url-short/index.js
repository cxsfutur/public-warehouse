...
async function handleRequest(request) {
  ...
  if(!path){
    ...
    const html= await fetch("https://url.mixao.cn/url-short/0/"+config.theme+"/index.html")
  ...
  if (location) {
    if (config.safe_browsing_api_key){
      if(!(await is_url_safe(location))){
        let warning_page = await fetch("https://url.mixao.cn/url-short/0/refuse.html")
    ...
    if (config.no_ref=="on"){
      let no_ref= await fetch("https://url.mixao.cn/url-short/0/noref.html")
